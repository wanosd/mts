		...
		<tr>
			...
			<c:choose>
				<c:when test="${entry.value > 5}">
					<td bgcolor="red"><font color="white"><c:out
								value="${entry.value}" /></font></td>
					<td>
						<form method="post"
							action="${pageContext.request.contextPath}/warnUser">
							<input type="hidden" value="${entry.key}" name="member" /> <input
								value="Warn User" type="submit" />
						</form>
					</td>
				</c:when>
				<c:otherwise>
					<c:choose>
						<c:when test="${entry.value > 2 and entry.value < 6}">
							<td bgcolor="orange"><font color="white"><c:out
										value="${entry.value}" /></font></td>
							<td><form method="post"
									action="${pageContext.request.contextPath}/warnUser">
									<input type="hidden" value="${entry.key}" name="member" /> <input
						...
						<c:otherwise>
							<td bgcolor="green"><font color="white"><c:out
										value="${entry.value}" /></font></td>
